...
{% for row in publist | selectattr("highlight", "equalto", 1) | batch(2) %}
<div class="row">
  {% for publi in row %}
  <div class="col-sm-6 clearfix">
    <div class="well">
      <pubtit>{{ publi.title }}</pubtit>
      <img src="/assets/images/pubpic/{{ publi.image }}" class="img-responsive" width="33%" style="float: left" />
...
<div class="full-list-section">
  ...
  <div class="search-container" style="margin-bottom: 30px; display: flex; gap: 10px;">
    ...
    <button onclick="resetSearch()" style="padding: 12px 20px; border-radius: 5px; border: 1px solid #ddd; background: #f8f9fa; cursor: pointer; font-weight: bold;">Reset</button>
  </div>
  ...
  <div id="fullPubList">
    ...
    <div class="pub-item" style="margin-bottom: 30px;">
      ...
      <div style="margin: 5px 0;">
        ...
        <a href="/{{ publi.research }}">{{ publi.research }}</a>
      </div>
      ...
      <a href="https://scholar.google.com/scholar?q=%22{{ publi.title | url_encode }}%22" target="_blank">.. Search Google Scholar</a>
...
function filterPublications() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    nodes[i].style.display = content.toLowerCase().indexOf(filter) > -1 ? 'block' : 'none';
  }
}
